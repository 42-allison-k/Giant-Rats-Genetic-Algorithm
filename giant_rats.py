...
def populate(num_rats, min_wt, max_wt, mode_wt):
    # initialize the population with a triangular distribution of weights.
...
def select(population, to_retain):
    # cull the population to retian only a specific number of memebrs.
...
def mutate(children, mutate_odds, mutate_min, mutate_max):
    # randomly alter weights using input odds & fractional changes.
    for i, rat in enumerate(children):
        if mutate_odds >= random.random():
            children[i] = round(rat * random.uniform(mutate_min, mutate_max))
    return children
...
def main():
    ...
    print("number to retian = {}".format(NUM_RATS))
    ave_wt = []
